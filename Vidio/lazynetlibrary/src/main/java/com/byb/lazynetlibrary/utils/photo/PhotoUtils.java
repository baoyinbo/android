package com.byb.lazynetlibrary.utils.photo;
...
public class PhotoUtils {
    ...
    public static void toCamera(Activity activity, File outputFile, int requestCode) {
        if (!isCameraUseAble(activity)) {
            Toast.makeText(activity, "不能使用相机", Toast.LENGTH_LONG).show();
            return;
        }
    ...
    public static void toAlbum(Activity activity, int requestCode) {
        try {
            ...
            Toast toast = Toast.makeText(activity, "不能使用相机", Toast.LENGTH_SHORT);
